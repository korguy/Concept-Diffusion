...
class AttendAndExcitePipeline(StableDiffusionPipeline):
    ...
    def get_indices(self, prompt: str) -> Dict[str, int]:
        ...
        indices = {tok[:-4]: i for tok, i in zip(self.tokenizer.convert_ids_to_tokens(ids), range(len(ids)))}
        return indices
...
if __name__ == "__main__":
    ...
    pipe = AttendAndExcitePipeline.from_pretrained("runwayml/stable-diffusion-v1-5").to('cuda')
    register_attention_control(pipe, attention_store)
    token_indices = [2, 5]
    prompt = "a cat and a frog"

    images = pipe(prompt=prompt, attention_store=attention_store, indices_to_alter=token_indices)
