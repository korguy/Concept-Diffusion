...
if __name__ == "__main__":
    ...
    config = yaml.safe_load(open("configs/models/composable.yaml", 'r'))

    scheduler = DDIMScheduler.from_pretrained(config["version"], subfolder="scheduler")
    model = ComposableStableDiffusionPipeline.from_pretrained(config["version"])
    model = model.to("cuda")

    img = model(prompt="cat|dog", **config['params']).images[0]